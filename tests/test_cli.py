...
class Cli(unittest.TestCase):
    def setUp(self):
        ...
        # claude finto: aggiunge una risposta alla sessione, come farebbe quello vero.
    ...
    def test_segui_from_the_project_folder(self):
        old = os.getcwd()
        os.chdir(self.work)
        try:
            self.assertIn("s1", self.run_cli("segui"))
        finally:
            os.chdir(old)
        self.assertIn("s1", load(self.home))

    def test_smetti(self):
        self.run_cli("segui", "s1")
        self.run_cli("smetti", "s1")
    ...
    def test_full_cycle_blocked_then_resumed_then_finished(self):
        ...
        self.assertEqual(load(self.home)["s1"].status, "bloccata")

        cli.tick(self.home, self.projects, datetime(2026, 9, 23, 9, 5, tzinfo=UTC))
        state = load(self.home)["s1"]
        self.assertEqual(state.status, "in ripresa")
        for _ in range(100):
        ...
        self.assertEqual(load(self.home)["s1"].status, "attiva")
        titles = [c.args[0] for c in self.notify.call_args_list]
        self.assertEqual(titles, ["Ripresa", "Finita"])

    def test_missing_claude_becomes_errore_and_segui_clears_it(self):
        self.run_cli("segui", "s1")
        with self.session.open("a") as f:
            f.write(json.dumps(LIMIT) + "\n")
        with mock.patch.dict(os.environ, {"PATH": "/nonexistent"}):
            cli.tick(self.home, self.projects, datetime(2026, 9, 23, 9, 5, tzinfo=UTC))
        self.assertEqual(load(self.home)["s1"].status, "errore")
        self.run_cli("segui", "s1")
        self.assertEqual(load(self.home)["s1"].status, "attiva")

    def test_stato_lists_sessions(self):
        self.run_cli("segui", "s1")
        self.assertIn(str(self.work), self.run_cli("stato"))
